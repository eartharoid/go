...
class JSTable {
	...
	_build() {
		...
		inner = inner.replace(
			'{pager}',
			'<div class=\'' + options.classes.pagination + ' is-centered is-small\'></div>'
		);
	...
	_updatePagination() {
		...
		pagination.appendChild(this.pager.render(pagination));
	}
...
class JSTablePager {
	...
	render(pagination) {
		...
		if (pages > 1) {
			...
			if (options.firstLast) {
				...
				item.className = 'pagination-previous';
				item.setAttribute('data-page', 1);
				item.innerHTML = options.firstText;
			...
			if (options.nextPrev) {
				let item = document.createElement('a');
				item.className = 'pagination-previous';
				item.setAttribute('data-page', prev);
				item.innerHTML = options.prevText;
				pagination.appendChild(item);
			}
			...
			if (options.nextPrev) {
				let item = document.createElement('a');
				item.className = 'pagination-next';
				item.setAttribute('data-page', next);
				item.innerHTML = options.nextText;
				pagination.appendChild(item);
			}
			...
			if (options.firstLast) {
				let item = document.createElement('a');
				item.className = 'pagination-next';
				item.setAttribute('data-page', pages);
				item.innerHTML = options.lastText;
				pagination.appendChild(item);
				// `<a class="pagination-next" data-page="${pages}"></a>`
			}
	...
	createItem(className, pageNum, content, ellipsis) {
		...
		item.innerHTML = !ellipsis ? `<a class="pagination-link${pageNum == this.instance.currentPage ? ' is-current' : ''}" data-page="${pageNum}">${content}</a>` : `<span class="pagination-ellipsis">${content}</span>`;
		return item;
	}
	...
	truncate() {
		...
		if (!this.instance.config.truncatePager) {
			for (let i = 1; i <= this.getPages(); i++) {
				pager.push(this.createItem(i === currentPage ? 'is-current' : '', i, i));
			}
			...
			range.forEach(function (index) {
				...
				pager.push(that.createItem(index == currentPage ? 'is-current' : '', index, index));
				lastIndex = index;
			});
		}
